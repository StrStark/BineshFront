...
export function LoginPage({ onLoginSuccess }: LoginPageProps) {
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.name === 'TypeError' && err.message === 'Failed to fetch') {
        errorMessage = "ارتباط با سرور برقرار نشد. لطفا اتصال اینترنت خود را بررسی کنید و دوباره تلاش کنید.";
      } else if (err.message) {
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.name === 'TypeError' && err.message === 'Failed to fetch') {
        errorMessage = "ارتباط با سرور برقرار نشد. لطفا دوباره تلاش کنید.";
      } else if (err.message) {
        errorMessage = err.message;
      }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (err.name === 'TypeError' && err.message === 'Failed to fetch') {
        errorMessage = "ارتباط با سرور برقرار نشد. لطفا اتصال اینترنت خود را بررسی کنید.";
      } else if (err.message) {
        errorMessage = err.message;
      }
