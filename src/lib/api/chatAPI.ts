...
export const chatAPI = {
  ...
  async getConversationMessages(conversationId: string): Promise<ChatMessagesResponse> {
    try {
      const data = await apiGet<ChatMessagesResponse>(`/api/chat/conversations/${conversationId}/messages`);
      return data;
    } catch (error: any) {
  ...
  async deleteConversation(conversationId: string): Promise<{ code: number; status: string; message: string }> {
    try {
      const data = await apiDelete<{ code: number; status: string; message: string }>(`/api/chat/conversations/${conversationId}`);
      return data;
    } catch (error: any) {
