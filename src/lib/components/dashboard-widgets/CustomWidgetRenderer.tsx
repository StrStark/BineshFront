...
export function CustomWidgetRenderer({ widget }: CustomWidgetRendererProps) {
  ...
  useEffect(() => {
    ...
    if (widgetAny.dataSource === "manual" && widgetAny.dataPoints) {
  ...
  useEffect(() => {
    const widgetAny = widget as any;
    if (!widgetAny.autoUpdate?.enabled) {
      return;
    }

    const intervalMs = widgetAny.autoUpdate.interval * 1000;
  ...
  }, [(widget as any).autoUpdate, fetchDataFromAPI]);
