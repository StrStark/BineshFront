...
export function SalesTrendWidget() {
  ...
  const [salesChartError, setSalesChartError] = useState<{ message: string; code?: number; details?: string } | null>(null);
    ...
    const fetchSalesData = async () => {
      ...
      try {
        ...
        if (response.code === 200 && response.status === "success") {
          ...
          setSalesChartError({ message: "خطا در دریافت داده‌های نمودار" });
        }
      } catch (err) {
