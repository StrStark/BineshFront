...
// --- AI Export Buttons for charts and tables ---

interface AIExportButtonsProps {
  showPdf?: boolean;
  showExcel?: boolean;
  title?: string;
  data?: any[];
...
export function AIExportButtons({ showPdf, showExcel, title, data, columns }: AIExportButtonsProps) {
  const colors = useCurrentColors();

  const handleExcelExport = () => {
    try {
      if (!data || data.length === 0) {
        console.warn("AIExportButtons: No data to export");
        return;
      }
      const ws = XLSX.utils.json_to_sheet(data);
      const wb = XLSX.utils.book_new();
      // Sheet name must be <= 31 chars and not contain special chars
      const safeSheetName = (title || "Sheet1").substring(0, 31).replace(/[\\/*?:\[\]]/g, "");
      XLSX.utils.book_append_sheet(wb, ws, safeSheetName || "Sheet1");
      
      // Use writeFile with fallback to blob download
      const safeFileName = (title || "export").replace(/[\\/*?:<>|"]/g, "");
      try {
      ...
      console.error("AIExportButtons: Excel export error:", error);
      alert("خطا در ایجاد فایل Excel. لطفاً دوباره تلاش کنید.");
    }
  };
  ...
  const handlePdfExport = () => {
    try {
      ...
      autoTable(doc, {
        ...
        headStyles: { fillColor: [41, 65, 122], halign: "right" },
        margin: { top: 15 },
        didDrawPage: () => {
          if (title) {
            doc.setFontSize(14);
      ...
      console.error("AIExportButtons: PDF export error:", error);
      alert("خطا در ایجاد فایل PDF. لطفاً دوباره تلاش کنید.");
    }
  };
...
// --- AI Copy Button for AI messages ---
...
export function AICopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      style={{
        backgroundColor: copied ? colors.success + "15" : colors.backgroundSecondary,
        color: copied ? colors.success : colors.textSecondary,
        border: `1px solid ${copied ? colors.success + "30" : colors.border}`,
      }}
